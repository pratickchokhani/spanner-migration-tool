...
type AutoGenCol struct {
	...
	// Additional options for auto generated columns with type IDENTITY, such as skip range
...
func (gc GeneratedColumn) PrintGeneratedColumn(ty Type) string {
	if !gc.IsPresent {
		return ""
	}
	var value string
	...
	case "FLOAT32", "NUMERIC", "BOOL", "BYTES":
		value = fmt.Sprintf(" AS (CAST(%s AS %s))", gc.Value.Statement, ty.Name)
	default:
		value = " AS (" + gc.Value.Statement + ")"
	}
	if gc.Type == GeneratedColStored {
		value += " " + string(GeneratedColStored)
	}
	return value
}
...
func (gc GeneratedColumn) PGPrintGeneratedColumn(ty Type) string {
	if !gc.IsPresent {
		return ""
	}
	var value string
	...
	case "FLOAT8", "FLOAT4", "REAL", "NUMERIC", "DECIMAL", "BOOL", "BYTEA":
		value = fmt.Sprintf(" GENERATED ALWAYS AS (CAST(%s AS %s))", gc.Value.Statement, GetPGType(ty))
	default:
		value = " GENERATED ALWAYS AS (" + gc.Value.Statement + ")"
	}
	value += " " + string(gc.Type)
	return value
}
...
func (agc AutoGenCol) PrintIdentityCol(c Config) string {
	...
	return fmt.Sprintf(" GENERATED BY DEFAULT AS IDENTITY (%s)", strings.Join(options, " "))
}
